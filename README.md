**VP Lead Software Engineer @ JPMorganChase** · London


**Libraries & tools**
- [`bonsai-js`](https://github.com/danfry1/bonsai-js) - fast, sandboxed expression evaluator
- [`vitest-native`](https://github.com/danfry1/vitest-native) - test React Native with Vitest, no Jest required
- [`reflow-ts`](https://github.com/danfry1/reflow-ts) - durable workflow execution for TypeScript
- [`faultline`](https://github.com/danfry1/faultline) - a complete type-safe error system for TypeScript
- [`typediff`](https://github.com/danfry1/typediff) - diff TypeScript types between package versions to verify semver
- [`theme-template`](https://github.com/danfry1/theme-template) - generate terminal & editor themes from one palette

**Apps**
- [`waxon`](https://github.com/danfry1/waxon) - a vim-modal Spotify client for the terminal
- [`marina`](https://github.com/danfry1/marina) - a developer-process cockpit for the terminal
- [`wordle-tui`](https://github.com/danfry1/wordle-tui) - a beautiful word-guessing game for the terminal

**Themes**
- [`lume`](https://github.com/danfry1/lume) - a muted, elegant dark color theme
- [`rime`](https://github.com/danfry1/rime) - an icy, near-black dark theme
- [`mind-the-gap`](https://github.com/danfry1/mind-the-gap) - a dark color theme inspired by the London Underground tube map
---

<p>🌐 <a href="https://danfry.dev">danfry.dev</a> · 𝕏 <a href="https://x.com/danfrydev">@danfrydev</a></p>
